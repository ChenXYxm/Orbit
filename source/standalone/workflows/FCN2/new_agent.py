...
class Push_Agent():
    ...
    def epsilon_greedy(self, state):
        ...
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                # # print(state.size())
                # image = np.zeros((self.WIDTH,self.WIDTH,3))
                # occu = np.squeeze(state.cpu().numpy())[0]
                # image[:,:,0] = occu.copy()
                # image[:,:,1] = output_np.copy()
                # fig, (ax1,ax2,ax3) = plt.subplots(1, 3, figsize=(15, 10))
                # occu[int(max_idx/self.WIDTH)-1:int(max_idx/self.WIDTH)+2,int(max_idx%self.WIDTH)-1:int(max_idx%self.WIDTH)+1] = 3
                ...
                print('predict action',max_idx//self.WIDTH,max_idx%self.WIDTH)
        ...
        # of the selected pixel and resample until you get a pixel corresponding to a point on the table
            ...
            self.last_action = "random"
            action = random.randrange(int(self.WIDTH*self.HEIGHT))
    ...
    def transform_action(self, action):
        ...
        action_1 = action_value // self.HEIGHT
        action_2 = action_value % self.WIDTH
        ...
        while self.steps_done<300000:
            ...
            for i in range(i_range):
                ...
                for j in range(len(self._last_obs)):
                    ...
                    actions[j,:2] = env_action.flatten()

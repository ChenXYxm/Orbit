...
class Push_Agent():
    ...
    def transform_action(self, action):
        ...
        action_1 = action_value // int(144*144)
        action_2 = (action_value % int(144*144))//144
        action_3 = (action_value % int(144*144))%144
        return np.array([action_2, action_3,action_1])
    def train(self):
        self.save_i = 1
        self._last_obs = self.env.reset()
        print('obs shape',self._last_obs.shape)
        i_range = int(np.ceil(4/len(self._last_obs)))
    ...
    def learn(self):
        ...
        for i in range(1):
            ...
            q_pred = (
                self.policy_net(state_batch).view(2, -1).gather(1, action_batch)
            )
            print('q_pred')
            ...
            q_next_state = self.target_net(next_state_batch).view(2, -1).max(1)[0].unsqueeze(1).detach()

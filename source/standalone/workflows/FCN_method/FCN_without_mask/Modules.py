...
class MULTIDISCRETE_RESNET_Rotate(nn.Module):
    ...
    def forward(self,state):
        ...
        for rotate_idx in range(rotate_num):
            ...
            # ax2.imshow(np.array(rotate_obs[0,1,:,:].clone().detach().cpu()))
            ...
            # ax3.imshow(np.array(obs_feature_rotated_back[0,0,:,:].clone().detach().cpu()))
            # plt.show()
            # max_value = tmp_tensor.view(-1).max(0)[0]
            # max_idx = tmp_tensor.view(-1).max(0)[1]
            # print('max idx',max_idx)
            # img_tmp = np.zeros((72,72,3))
            # img_tmp[:,:,1] = 100
            # img_tmp[max_idx//72,max_idx%72,1] = 255
            ...
            # img_tmp[max_idx//72,max_idx%72,0] = 255
            # img_tmp[max_idx//72,max_idx%72,1] = 255

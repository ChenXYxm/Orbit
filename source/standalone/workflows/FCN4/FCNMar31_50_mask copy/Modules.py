...
def conv3x3_2(in_planes, out_planes, stride=1, groups=1, dilation=1):
...
class Perception_Module(nn.Module):
    def __init__(self):
        ...
        #######################################
        self.mask_RB0 = BasicBlock(1,32)
        self.mask_RB1 = BasicBlock(32,64)
        self.mask_MP0 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.mask_RB2 = BasicBlock(64,128)
        self.mask_RB3 = BasicBlock(128,128)
        self.mask_MP1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.mask_RB4 = BasicBlock(128,256)
        
    def forward(self, x, mask,verbose=0):
        if verbose == 1:
        ...
        mask = self.mask_RB0(mask)
        mask = self.mask_RB1(mask)
        mask = self.mask_MP0(mask)
        mask = self.mask_RB2(mask)
        mask = self.mask_RB3(mask)
        mask = self.mask_MP1(mask)
        mask = self.mask_RB4(mask)
        x =torch.cat((x,mask), dim=1)
        # print('mask shape',mask.size())
        # mask = self.mask_RB3(mask)
        # mask = self.mask_MP2(mask)
        # print('output size of feature net',x.size(),x_c.size())
        return x,x_c
...
class Grasping_Module(nn.Module):
    ...
    def forward(self, x, verbose=0):
        ...
        if self.output_activation is not None:
            ...
            return x
'''
class Placing_Perception_Module(nn.Module):
    def __init__(self):
        super(Placing_Perception_Module, self).__init__()
        # self.C1 = conv3x3(1, 64)
        self.C0 = conv5x5(2, 32)
        self.C1 = conv3x3(32, 64)
        self.relu = nn.ReLU()
        ...
        self.RB1 = BasicBlock(64, 128)
        self.MP2 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.RB2 = BasicBlock(128, 128)
        # self.RB3 = BasicBlock(256, 512)
        ##### modified in Feb11
        self.MP3 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.RB4 = BasicBlock(128, 128)
        # self.RB5 = BasicBlock(1024, 1024)
        #######################################
        self.mask_RB0 = BasicBlock(1,32)
        self.mask_RB1 = BasicBlock(32,64)
        ...
        # self.mask_RB3 = BasicBlock(128,128)
        self.mask_MP1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.mask_RB4 = BasicBlock(128,128)
    ...
    def forward(self, x, mask,verbose=0):
        ...
        # print('output size of feature net',x.size(),x_c.size())
        return x,x_c
class Placing_Module_multidiscrete(nn.Module):
    def __init__(self, output_activation="Sigmoid", act_dim_2=1):
        super(Placing_Module_multidiscrete, self).__init__()
        #############################Feb 11
        # self.RB01 = BasicBlock(1024, 1024)
        self.RB00 = BasicBlock(256, 256)
        # self.RB02 = BasicBlock(1024, 512)
        self.UP0 = nn.UpsamplingBilinear2d(scale_factor=2)
        #############################
        self.RB1 = BasicBlock(256, 128)
        # self.RB2 = BasicBlock(256, 128)
        self.UP1 = nn.UpsamplingBilinear2d(scale_factor=2)
        self.RB3 = BasicBlock(256, 128)
        self.RB4 = BasicBlock(128, 64)
        self.UP2 = nn.UpsamplingBilinear2d(scale_factor=2)
        # self.C1 = nn.Conv2d(64, 64, kernel_size=1)
        self.C2 = nn.Conv2d(64, act_dim_2, kernel_size=1)
        self.relu = nn.ReLU()
    ...
    def forward(self, x, x_c,verbose=0):
        if verbose == 1:
            print("### Grasping Module ###")
            print("Input: ".ljust(15), x.size())
        ################################### Feb 11
...
class Grasping_Module_multidiscrete(nn.Module):
    def __init__(self, output_activation="Sigmoid", act_dim_2=1):
        ...
        #############################Feb 11
        # self.RB01 = BasicBlock(1024, 1024)
        self.RB00 = BasicBlock(1280, 1024)
        self.RB02 = BasicBlock(1024, 512)
        self.UP0 = nn.UpsamplingBilinear2d(scale_factor=2)
        #############################
        self.RB1 = BasicBlock(512, 256)
...
class MULTIDISCRETE_RESNET_Rotate(nn.Module):
    def __init__(self, number_actions_dim_2=1): 
        ...
        # )
        self.placing_feature_net = Placing_Perception_Module()
        self.placing_process_net = Placing_Module_multidiscrete(act_dim_2=number_actions_dim_2)
        if torch.cuda.is_available():
            self.use_cuda = True
            # self.net.to('cuda')
            self.feature_net.to('cuda')
            self.process_net.to('cuda')
            self.placing_feature_net.to('cuda')
            self.placing_process_net.to('cuda')
        else:
            self.use_cuda = False
    def forward(self,state,mask):
        ...
        rotate_num = 8
        self.output = torch.zeros((len(state),int(rotate_num*2),72,72),device='cuda')
        for rotate_idx in range(rotate_num):
            ...
            # plt.show()   
            obs_feature,x_c = self.feature_net(rotate_obs,mask)
            obs_feature = self.process_net(obs_feature,x_c)
            # obs_feature = self.net(rotate_obs,mask)
        ...
        for rotate_idx in range(rotate_num):
            ...
            # plt.imshow(np.array(rotate_obs[0,0].cpu()))
            # plt.show()   
            obs_feature,x_c = self.placing_feature_net(rotate_obs,mask)
            obs_feature = self.placing_process_net(obs_feature,x_c)
            # obs_feature = self.net(rotate_obs,mask)
            if obs_feature.dim() == 2:
                tmp_tensor = torch.zeros((1,1,72,72),device='cuda')
                tmp_tensor[0,0,:,:] = obs_feature
            ...
            # print('obs_rotate_bask_size: ',obs_feature_rotated_back.size())
            # self.output_prob.append(obs_feature_rotated_back)
            self.output[:,rotate_idx+8,:,:] = obs_feature_rotated_back[:,0,:,:]
        return self.output

...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        # checkpoint = torch.load('/home/cxy/Downloads/Mar31/Mar28_mask_weight128000.pth')
    ...
    def epsilon_greedy(self, state,mask_o,env_idx):
        ...
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                for i in range(12):
                    mask = mask_o.copy()
                    mask = rotate_45(mask,degree=-i*30)
                    mask = mask[7:43,7:43]
                    ...
                    if final_max_value<float(max_value.cpu()):
                ...
                # print(output.size())
                # print(max_idx)
                print('value',final_max_value,'action:',int(final_max_idx//(self.WIDTH*self.WIDTH)),int(final_max_idx%(self.WIDTH*self.WIDTH)//self.WIDTH),int(final_max_idx%(self.WIDTH*self.WIDTH)%self.WIDTH))
                # print(max_value,output_np[int(max_idx/self.WIDTH),int(max_idx%self.WIDTH)])
                # print('####################################')

                # print('predict action',max_idx)
                return final_max_idx.unsqueeze_(0).cpu(),mask_tensor
        # else:
        #     self.last_action = 'random'
        #     return torch.tensor([[random.randrange(self.output)]], dtype=torch.long)
        ...
        # of the selected pixel and resample until you get a pixel corresponding to a point on the table
            ...
            mask_rot = random.randrange(12)
            mask = mask_o.copy()
            mask = rotate_45(mask,degree=-mask_rot*30)
            mask = mask[7:43,7:43]
            mask = mask[:, :, np.newaxis]
            mask = np.moveaxis(mask, -1, 0)
        ...
        while self.steps_done<300000:
            ...
            for i in range(i_range):
                ...
                for j in range(len(self._last_obs)):
                    ...
                    action,mask_tensor = self.epsilon_greedy(obs_tensor,mask_tmp,j)

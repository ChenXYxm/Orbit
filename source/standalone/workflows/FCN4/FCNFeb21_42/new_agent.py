...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        checkpoint = torch.load('/home/chenxiny/orbit/Orbit/FCN_regression/weight30000.pth')
        # checkpoint = torch.load('/home/cxy/Thesis/orbit/Orbit/FCN_regression/weight99000.pth')
    ...
    def epsilon_greedy(self, state,mask):
        ...
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                # max_idx_2 = max_idx%int(64*64)
                # image = np.zeros((64,64,3))
                # occu = np.squeeze(state.cpu().numpy())[0]
                # image[:,:,0] = occu.copy()
                # image[:,:,1] = output_np.copy()
                # fig, (ax1,ax2,ax3) = plt.subplots(1, 3, figsize=(15, 10))
                # occu[int(max_idx_2/64)-4:int(max_idx_2/64)+5,int(max_idx_2%64)-3:int(max_idx_2%64)+4] = 3

...
def update_weight(pretrained_dict1,pretrained_dict0,updated_model_dict):
    for i in pretrained_dict1:
        # print(i)
        if i in updated_model_dict:
                print('update '+i)
                updated_model_dict[i].copy_(pretrained_dict1[i])
    for i in pretrained_dict0:
        # print(i)
        if i in updated_model_dict:
                print('update '+i)
                updated_model_dict[i].copy_(pretrained_dict0[i])
...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        # pretrained_dict = torch.load('/home/cxy/Downloads/Mar31/Mar28_mask_weight128000.pth')
        self.saved_changed = {}
        for i in range(self.num_envs):
            self.saved_changed[i] = []
        # print(self.saved_changed)
        checkpoint1 = torch.load('/home/cxy/Thesis/orbit/Orbit/FCN_regression/Mar28_mask_weight83200.pth')
        checkpoint0 = torch.load('/home/cxy/Downloads/Mar28_mask_weight96000.pth')
        ...
        # ''' 
        updated_model_dict = self.policy_net.state_dict()
        updated_model_dict=update_weight(checkpoint1,checkpoint0,updated_model_dict)
    ...
    def epsilon_greedy(self, state,mask,env_idx):
        ...
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                final_max_index = 0
                for i in range(12):
                    if i in self.saved_changed[env_idx]:
                        continue
                    mask_t = mask_tmp.copy()
                    mask_t = rotate_45(mask_t,degree=30*i)
                    mask_t = mask_t[:, :, np.newaxis]
                    mask_t = np.moveaxis(mask_t, -1, 0)
                    mask_tensor = self.transform_mask(mask_t)
                    ...
                    if max_value_final<float(max_value.cpu()):
                        max_value_final = float(max_value.cpu())
                        final_mask_tensor = mask_tensor.clone().cpu()
                        final_orientaion = i
                        final_max_index = max_idx.clone()
                
                # self.saved_changed[env_idx].append(int(final_orientaion))
                ...
                # print(output.size())
                # print(max_idx)
                print('value',max_value_final,'action:',final_orientaion,int(final_max_index//(self.WIDTH*self.WIDTH)),int(final_max_index%(self.WIDTH*self.WIDTH)//self.WIDTH),int(final_max_index%(self.WIDTH*self.WIDTH)%self.WIDTH))
                # print(max_value,output_np[int(max_idx/self.WIDTH),int(max_idx%self.WIDTH)])
                # print('####################################')

                # print('predict action',max_idx)
                return final_max_index.unsqueeze_(0).cpu(),final_orientaion,final_mask_tensor
        # else:
        #     self.last_action = 'random'
        #     return torch.tensor([[random.randrange(self.output)]], dtype=torch.long)
        ...
        # of the selected pixel and resample until you get a pixel corresponding to a point on the table
            ...
            final_orientaion = random.randrange(12)
            mask_t = mask_tmp.copy()
            mask_t = rotate_45(mask_t,degree=30*final_orientaion)
            mask_t = mask_t[:, :, np.newaxis]
            mask_t = np.moveaxis(mask_t, -1, 0)
            mask_tensor = self.transform_mask(mask_t)
            return torch.tensor([[action]], dtype=torch.long),final_orientaion,mask_tensor
        ...
        while self.steps_done<300000:
            ...
            for i in range(i_range):
                ...
                for j in range(len(self._last_obs)):
                    ...
                    mask_tmp = self.env.obj_masks[j].copy()
                    # mask_o = mask_tmp.copy()
                    
                        # plt.imshow(mask_t)
                        # plt.show()
                    mask_tmp = mask_tmp[7:43,7:43]
                    # print('mask get by agent')
                    # plt.imshow(mask_tmp)
                    # plt.show()
                    ...
                    action,orientation,mask_tensor = self.epsilon_greedy(obs_tensor,mask_tmp,j)
                    ...
                    if actions[j,2] > 7:
                        actions[j,2] += orientation
                    # actions[j,2] = orientation+8
                ...
                for idx, done in enumerate(dones):
                    ...
                    if done:
                        ...
                        if self.env.check_reaching[idx]>0.5 or self.env.place_success[idx]>0.5:
                            self.saved_changed[idx] = []
                        # print('not terminate')
                    # if done:
                    #     print('terminate')
            ...
            self.learn()
            if self.steps_done>=6400*self.save_i:
                self.save_i += 1
                print('num model to be saved: ',self.save_i)
                torch.save(self.policy_net.state_dict(),'FCN_regression/Mar28_mask_weight'+str(self.steps_done)+'.pth')

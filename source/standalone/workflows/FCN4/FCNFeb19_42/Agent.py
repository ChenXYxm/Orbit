...
class Push_Agent:
    ...
    def __init__(
        ...
        if train:
            ...
            # )
            self.env = gym.make(args_cli.task, cfg=env_cfg, headless=args_cli.headless, viewport=args_cli.video)
        ...
        if train:
            ...
            self.writer.add_graph(
                self.policy_net, torch.zeros(1, 2, self.WIDTH, self.HEIGHT).to(device)
            )
    ...
    def epsilon_greedy(self, state):
        ...
        # of the selected pixel and resample until you get a pixel corresponding to a point on the table
            ...
            x = action_1 % self.WIDTH
            y = action_1 // self.WIDTH
            # while True:
            #     action = random.randrange(self.output)
            #     action_1 = action % self.n_actions_1
            #     x = action_1 % self.WIDTH
    ...
    def transform_observation(self, observation, normalize=True, jitter_and_noise=True):
        ...
        obs = obs/255.0
        obs += np.random.normal(loc=0, scale=1.0, size=obs.shape)
...
def main():
    ...
    for rand_seed in [999]:
        for lr in [0.0005]:
            ...
            agent = Push_Agent(seed=rand_seed, load_path=None, learning_rate=lr, depth_only=False)

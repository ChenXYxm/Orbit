...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        # checkpoint = torch.load('/home/chenxiny/orbit/Orbit/FCN/weight48006.pth')
        # checkpoint = torch.load('/home/cxy/Thesis/orbit/Orbit/FCN_regression/weight18000.pth')
    ...
    def epsilon_greedy(self, state,mask):
        ...
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                # print('greedy')
                # print(output_np[np.unravel_index(np.argmax(output_np),output_np.shape)],np.max(output_np))
                # output_np[np.unravel_index(np.argmax(output_np),output_np.shape)] = 1.5
                # plt.imshow(output_np[0,max_1])
                # plt.show()
                # print('####################################')
                # print(output.size())
                # print(max_idx)
    ...
    def learn(self):
        ...
        for i in range(1):
            ...
            q_pred = (
                self.policy_net(state_batch,mask_batch).view(2, -1).gather(1, action_batch)
            )
            # print('q_pred')
            # print(self.policy_net(state_batch,mask_batch).size())
            # q_pred = self.policy_net(state_batch).gather(1, action_batch)
            print('q_pred',q_pred.size())

            q_next_state = self.target_net(next_state_batch,mask_batch).view(2, -1).max(1)[0].unsqueeze(1).detach()
            print('q_pred',q_next_state.size())

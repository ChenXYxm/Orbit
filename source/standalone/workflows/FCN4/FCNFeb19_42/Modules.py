...
class Perception_Module(nn.Module):
    ...
    def forward(self, x_o,x_m, verbose=0):
        ...
        x = self.relu(x)
        if verbose == 1:
            print("After C5: ".ljust(15), x.size())
        # x= self.bn2(x)
        # x = self.RB5(x)
        # if verbose == 1:
        #     print("After RB3: ".ljust(15), x.size())
...
class MULTIDISCRETE_RESNET(nn.Module):
    ...
    def forward(self,x,m):
        ...
        m = m
        self.output = torch.zeros((len(obs_tmp),1,64,64),device='cuda')
        for rotate_idx in range(1):
            ...
            # print('size of network output',obs_feature.size())
            ...
            # print('rotated back feature',obs_feature_rotated_back.size())
            # print('ori max',torch.max(obs_tmp[0,0,:,:]),torch.max(obs_tmp[0,1,:,:]))

...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        # checkpoint = torch.load('/home/chenxiny/Downloads/Mar1_env_and_agent/weight5400.pth')
        checkpoint = torch.load("/home/chenxiny/orbit/Orbit/FCN_regression/weight22000.pth")
        ...
        self.last_orient = np.ones(num_envs)
    ...
    def epsilon_greedy(self, state,idx):
        ...
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                # print(output.size())
                if done == 0 and reward == 0 and self.last_greedy[idx]==0:
                    output[:,int(last_ori)] = float(torch.min(output))
        ...
        while self.steps_done<300000:
            ...
            for i in range(i_range):
                ...
                for j in range(len(self._last_obs)):
                    ...
                    self.last_orient[j] = int(actions[j,2]/int(np.ceil(8.0/self.Orient)))
            ...
            self.learn()
            if self.steps_done>=2000*self.save_i:
                self.save_i += 1

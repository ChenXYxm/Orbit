...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        # checkpoint = torch.load('/home/chenxiny/Downloads/Mar24/weight37800.pth')
        # checkpoint = torch.load('/home/chenxiny/orbit/Orbit/FCN_regression/weight9600.pth')
        # checkpoint = torch.load('/local/home/chenxiny/Downloads/weight12000.pth')
        # checkpoint = torch.load('/home/chenxiny/Downloads/weight12000.pth')
        ...
        self.save_changed = [1 for i in range(self.num_envs)]
        self.saved_ori_failed = {}
        for i in range(self.num_envs):
            self.saved_ori_failed[i] = []
        print('self.save_changed',self.save_changed)
    ...
    def epsilon_greedy(self, state,env_id):
        """
        ...
        """
        # print(self.steps_done//2+1,self.save_changed,self.saved_ori_failed)
        ...
        # if sample > self.eps_threshold and self.save_changed[env_id]>=0.5:
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                # print(output.size())
                for i in self.saved_ori_failed[env_id]:
                    output[:,i,:,:] = torch.min(output)
                ...
                # print(output.size())
                # print(max_idx)
                self.saved_ori_failed[env_id].append(int(max_idx//(self.WIDTH*self.WIDTH)))
            ...
            for i in range(i_range):
                ...
                for idx, done in enumerate(dones):
                    ...
                    next_obs_tmp = self.env.tabletop_scene_tmp[idx].copy()
                     # next_obs_tmp = self.env.tabletop_scene_tmp[idx].copy()
                    # print('next obs')
                    # plt.imshow(next_obs_tmp[:,:,0])
                    # plt.show()
                    if done:
                        self.save_changed[idx] = 1
                        self.saved_ori_failed[idx] = []
                    else:
                        if self.env.check_reaching[idx] >=0.5:
                            self.save_changed[idx] = 1
                            self.saved_ori_failed[idx] = []
                        elif self.save_changed[idx]>=0.5 and self.last_action=='greedy' and self.env.check_reaching[idx] <0.5:
                            self.save_changed[idx] = 0
                    # if done:
                    #     print('terminate')
                ...
                print(self.steps_done//self.num_envs,self.save_changed,self.saved_ori_failed)

...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        self.BATCH_SIZE = 8
        self.GAMMA = 0.9
        self.policy_net = MULTIDISCRETE_RESNET_Rotate(number_actions_dim_2=1).to(self.device)
        # Only need a target network if gamma is not zero
        self.target_net = MULTIDISCRETE_RESNET_Rotate(number_actions_dim_2=1).to(self.device)
    ...
    def epsilon_greedy(self, state):
        ...
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                # occu[int(max_idx/64)-1:int(max_idx/64)+2,int(max_idx%64)-1:int(max_idx%64)+1] = 3

...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        # checkpoint = torch.load('/home/cxy/Thesis/orbit/Orbit/FCN_regression/weight15000.pth')
        ...
        self.check_reaching_orien = dict()
        for i in range(self.num_envs):
            self.check_reaching_orien[i] = 0
        if self.training:
    ...
    def epsilon_greedy(self, state,env_idx):
        """
        ...
        if sample > self.eps_threshold and self.check_reaching_orien[env_idx]==0:
            ...
            with torch.no_grad():
                ...
                for i in range(self.Orient):
                    # print('greedy')
                    ori = np.array([i])
                    output = self.policy_net(state.to(self.device),ori)
        ...
        while self.steps_done<300000:
            ...
            for i in range(i_range):
                ...
                for j in range(len(self._last_obs)):
                    ...
                    action,orientation = self.epsilon_greedy(obs_tensor,j)
                    # print("action: ",action)
                ...
                for j in range(len(self._last_obs)):
                    if self.env.check_reaching[j]<=0.5 and self.last_action == "greedy":
                        self.check_reaching_orien[j] = 1
                    elif self.env.check_reaching[j]>=0.5:
                        self.check_reaching_orien[j] = 0
                for idx, done in enumerate(dones):
                    ...
                    if done:
                        self.check_reaching_orien[idx] = 0
                        # print('not terminate')
                    # if done:
                    #     print('terminate')
                ...
                # for j in range(len(self._last_obs)):
                    ...
                    self.memory.push(obs_tmp,action_tmp,next_obs_tensor,rewards_tmp,orientation_tmp)
                    # print('check_memory')
            ...
            if self.steps_done>=2400*self.save_i:
                ...
                # if self.save_i%2 == 0:
                #     self.GAMMA -=0.01
                #     print(self.save_i,'gama value: ',self.GAMMA)
    ...
    def learn(self):
        ...
        for i in range(2):
            ...
            q_pred = (
                self.policy_net(state_batch,orientation_batch).view(self.BATCH_SIZE//2, -1).gather(1, action_batch)
            )
            # print('orient',orientation_batch,action_batch)
            ...
            q_next_state = self.target_net(next_state_batch,tmp_orientation).view(self.BATCH_SIZE//2, -1).max(1)[0].unsqueeze(1).detach()
            # print(q_next_state.size(),q_next_state)
            for j in range(1,self.Orient):
                tmp_orientation = np.ones(self.BATCH_SIZE//2)*j
                q_next_state_tmp = self.target_net(next_state_batch,tmp_orientation).view(self.BATCH_SIZE//2, -1).max(1)[0].unsqueeze(1).detach()
                for k in range(self.BATCH_SIZE//2):
                    if float(q_next_state_tmp[k])>float(q_next_state[k]):
                        q_next_state[k] = q_next_state_tmp[k]
            # print('q_next shape',q_next_state_tmp.size(),q_next_state)

...
Transition = namedtuple("Transition", ("state", "action", "next_state", "reward","mask","orientation"))
...
class Perception_Module(nn.Module):
    def __init__(self):
        ...
        #######################################
        self.mask_RB0 = BasicBlock_2(1,32)
        self.mask_RB1 = BasicBlock_2(32,64)
        self.mask_RB2 = BasicBlock_2(64,64)
        
        self.mask_MP1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        
        self.mask_RB3 = BasicBlock(64,128)
        # self.mask_RB3 = BasicBlock(64,128)
        # self.mask_MP2 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
class Grasping_Module_multidiscrete(nn.Module):
    def __init__(self, output_activation="Sigmoid", act_dim_2=1):
        ...
        #############################Feb 11
        self.RB00 = BasicBlock(1152, 1024)
...
class MULTIDISCRETE_RESNET_Rotate(nn.Module):
    def __init__(self, number_actions_dim_2=1): 
        ...
        if torch.cuda.is_available():
            ...
            self.use_cuda = False
    def forward(self,state,mask,rotatation):
        
        self.output = torch.zeros((len(state),1,72,72),device='cuda')
        affine_mat_before_tmp = np.zeros((2,3,len(state)))
        affine_mat_after_tmp = np.zeros((2,3,len(state)))
        for i in range(len(state)):
            rotate_idx = int(rotatation[i])
        ...
        del affine_mat_before_tmp, affine_mat_after_tmp
        if torch.cuda.is_available():
        ...
        del affine_mat_before,flow_grid_before
        obs_feature = self.feature_net(rotate_obs,mask)
        obs_feature = self.process_net(obs_feature)
        ...
        self.output[:,0,:,:] = obs_feature_rotated_back[:,0,:,:]

        # max_ind = tmp_tensor[0,0].view(-1).max(0)[1]
        # print('ori max',max_ind)
        # rotate_obs[0,0,int(max_ind//72),int(max_ind%72)] = 2
        ...
        del rotate_obs, tmp_tensor,obs_feature_rotated_back
        return self.output

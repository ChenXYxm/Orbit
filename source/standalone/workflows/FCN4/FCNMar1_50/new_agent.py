...
class Push_Agent():
    def __init__(self,env,num_envs,device) -> None:
        ...
        checkpoint = torch.load('/home/cxy/Thesis/orbit/Orbit/FCN_regression/weight50_90024.pth')
        ...
        self.last_reward=np.zeros(num_envs)
        self.last_done = np.ones(num_envs)
        self.last_orient = dict()
        for i in range(num_envs):
            self.last_orient[i] = []
        self.last_greedy = np.zeros(num_envs)
        if self.training:
    ...
    def epsilon_greedy(self, state,idx):
        ...
        done = self.last_done[idx]
        reward = self.last_reward[idx]
        last_ori = self.last_orient[idx]
        ...
        if sample > self.eps_threshold:
            ...
            with torch.no_grad():
                ...
                # print(output.size())
                # if done == 0 and reward == 0 and len(last_ori)>0:
                #     # print('remove repeat',self.last_orient)
                #     for last_ori_i in last_ori:
                #         output[:,int(last_ori_i)] = float(torch.min(output))
                if done == 1 or reward > 0:
                    # print('clean repeat',self.last_orient)
                    self.last_orient[idx] = []
                    # print('clean repeat',self.last_orient)
                    # plt.imshow(output[0,int(last_ori)].cpu().detach().numpy())
                    # plt.show()
    ...
    def train(self):
        ...
        while self.steps_done<300000:
            ...
            # while self.new_data <self.BATCH_SIZE*2/3:
                ...
                for j in range(len(self._last_obs)):
                    ...
                    #     self.last_orient[j].append(int(actions[j,2]/int(np.ceil(8.0/self.Orient))))
                ...
                for idx, done in enumerate(dones):
                    if (
                        ...
                        self.last_done[idx] = 1
                    else:
                        if not done:
                            self.last_done[idx] = 0
                        else:
                            self.last_done[idx] = 1
                ...
                # for j in range(len(self._last_obs)):
                    ...
                    if rewards.flatten()[idx]>-0.008 and rewards.flatten()[idx]<-0.007:
                        self.negative_reward += 1
                        
                    else:
                        self.negative_reward = 0
                    # if self.negative_reward>10:
                    #     self.eps_threshold = 0.3
                    #     if self.negative_reward>20:
                    #         self.eps_threshold = 0.5

                    # mask_tmp = mask_tensor_list[idx]
                    # if self.negative_reward<=3:
                    ...
                    # plt.show()
                    self.last_reward[idx]=float(rewards_tmp)
            ...
            if flag:
                if self.steps_done>=6000*self.save_i:
                    self.save_i = np.ceil(self.steps_done/6000.0)

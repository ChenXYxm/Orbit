...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # for i in skip_connections:
            # print('oringin skip',i.size())
        skip_connections = skip_connections[:-1][::-1]  
        # for i in skip_connections:
            # print('final skip',i.size())
        for i in range(0, len(self.decoder), 2):
            x = self.decoder[i](x)
            # print('x shape',x.size())
            skip_connection = skip_connections[i]
            # print('skip shape',skip_connection.size())
            ...
            # print('cat shape',concat_skip.size())
...
class Grasping_Module_multidiscrete(nn.Module):
    def __init__(self, output_activation="Sigmoid", act_dim_2=1):
        ...
        self.RB01 = BasicBlock(2048, 1024)
...
class MULTIDISCRETE_RESNET_Rotate(nn.Module):
    ...
    def forward(self,state):
        ...
        print('len states',len(state))
        for rotate_idx in range(rotate_num):
                # rotate_theta = -np.radians(int(rotation[j])*(45))

                # # Compute sample grid for rotation BEFORE neural network
                # affine_mat_before = np.asarray([[np.cos(-rotate_theta), np.sin(-rotate_theta), 0],[-np.sin(-rotate_theta), np.cos(-rotate_theta), 0]])
                # affine_mat_before.shape = (2,3)
                # affine_mat_before_tmp = np.zeros((2,3,len(state_tmp)))
                ...
                # # print(obs_tmp.size())
                # if torch.cuda.is_available():
                #     flow_grid_before = F.affine_grid(Variable(affine_mat_before, requires_grad=False).cuda(), state_tmp.size(),align_corners = False)
                # else:
                #     flow_grid_before = F.affine_grid(Variable(affine_mat_before, requires_grad=False), state_tmp.size(),align_corners = False)
                # if self.use_cuda:
                #     rotate_obs = F.grid_sample(Variable(state_tmp, requires_grad=False).cuda(), flow_grid_before, mode='nearest',align_corners = False)
                    
                # else:
                #     rotate_obs = F.grid_sample(Variable(state_tmp, requires_grad=False), flow_grid_before, mode='nearest',align_corners = False)
                # # plt.imshow(np.array(rotate_obs[0,0].cpu()))
                # # plt.show()   
                obs_feature = self.net(state)
                # self.output[:,rotate_idx,:,:] = tmp_tensor[:,0,:,:]
                # obs_feature = self.net(state)
                print('network output',obs_feature.size(),state.size())
                if obs_feature.dim() == 2:
                    tmp_tensor = torch.zeros((1,1,64,64),device='cuda')
                    tmp_tensor[0,0,:,:] = obs_feature
                elif obs_feature.dim() == 4:
                    tmp_tensor = obs_feature
                else:
                ...
                # affine_mat_after_tmp = np.zeros((2,3,len(state_tmp)))
                # for i in range(len(state_tmp)):
                ...
                # self.output[j,rotate_idx,:,:] = obs_feature_rotated_back[:,0,:,:]
                # self.output[j,rotate_idx,:,:] = tmp_tensor[:,0,:,:]

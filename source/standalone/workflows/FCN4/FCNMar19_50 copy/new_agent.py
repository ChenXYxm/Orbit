...
class Push_Agent():
    ...
    def learn(self):
        ...
        for i in range(2):
            ...
            q_pred = (
                self.policy_net(state_batch,mask_batch).view(self.BATCH_SIZE//2, -1).gather(1, action_batch)
            )
            # print('q_pred')
            ...
            q_next_state = self.target_net(next_state_batch,mask_batch).view(self.BATCH_SIZE//2, -1).max(1)[0].unsqueeze(1).detach()
            # print('q_pred')

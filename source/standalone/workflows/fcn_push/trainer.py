...
class Trainer(object):
    def __init__(self, method, push_rewards, future_reward_discount,
        ...
        if self.method == 'reinforcement':
            self.model = FCNet(self.use_cuda)
    ...
    def forward(self, depth_heightmap):
        ...
        depth_heightmap_2x = depth_heightmap.clone()
        ...
        output_prob = self.model.forward(input_depth_data)

        push_predictions = output_prob[0][0].cpu().data.numpy()
    ...
    def get_label_value(self, primitive_action, push_success, grasp_success, change_detected, prev_push_predictions, prev_grasp_predictions, next_color_heightmap, next_depth_heightmap):
        ...
        if self.method == 'reinforcement':
            ...
            if not change_detected and not grasp_success:
                ...
                next_push_predictions= self.forward(next_depth_heightmap)

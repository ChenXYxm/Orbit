...
class NatureCNN(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 512):
        ...
        '''
        self.cnn = nn.Sequential(
            nn.Conv2d(n_input_channels, 32, kernel_size=3, stride=1, padding=1,dilation=1),
            nn.LeakyReLU(),
            nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=0),
            nn.Dropout(p=0.1),
            #nn.BatchNorm2d(32), ## new
            nn.LeakyReLU(),
            #nn.MaxPool2d(kernel_size=3, stride=2, padding=1), ##new
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=0,dilation=1),
            nn.Dropout(p=0.1),
            nn.LeakyReLU(),
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=0,dilation=1),
            nn.Dropout(p=0.1),
            nn.LeakyReLU(),
            #nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=0), ##new
            #nn.Dropout(p=0.1),
...
class NatureCNN_Customize(BaseFeaturesExtractor):
    """
    CNN from DQN Nature paper:
        Mnih, Volodymyr, et al.
        "Human-level control through deep reinforcement learning."
    ...
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 512):
        ...
        # We assume CxHxW images (channels first)
        # Re-ordering will be done by pre-preprocessing or wrapper
        mask_space = gym.spaces.Box(low=0, high=255, shape=(32,32,1),dtype=np.uint8)
        assert is_image_space(observation_space, check_channels=False), (
            "You should use NatureCNN "
            f"only with images not with {observation_space}\n"
            "(you are probably using `CnnPolicy` instead of `MlpPolicy` or `MultiInputPolicy`)\n"
        ...
        # Compute shape by doing one forward pass
        with th.no_grad():
            n_flatten = self.cnn(th.as_tensor(observation_space.sample()[None]).float()).shape[1] + self.cnn_mask(th.as_tensor(mask_space.sample()[None]).float()).shape[1]

        self.linear = nn.Sequential(nn.Linear(n_flatten, features_dim), nn.ReLU())

    def forward(self, observations: th.Tensor,mask:th.Tensor) -> th.Tensor:
        return self.linear(th.cat((self.cnn(observations),self.cnn_mask(mask)),1))
def create_mlp(
    input_dim: int,
    output_dim: int,
    net_arch: List[int],

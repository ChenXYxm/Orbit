...
class BaseModel(nn.Module):
    ...
    def extract_features(self, obs: th.Tensor) -> th.Tensor:
        ...
        #print('normilize')
        #print(th.max(preprocessed_obs),th.min(preprocessed_obs))
        #print(preprocessed_obs.size())
        #obs_tmp =preprocessed_obs.clone().cpu().numpy()
        #for i in range(len(obs_tmp)):
            #plt.imshow(np.squeeze(obs_tmp[i]))
            #plt.show()
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        if net_arch is None:
            if features_extractor_class == NatureCNN:
                net_arch = []
            elif features_extractor_class == NatureCNN_Customize:
                net_arch = []
            else:
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor) -> Distribution:
        ...
        #print('mean actions size',mean_actions.size())
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            #print('MultiCategoricalDistribution')
            return self.action_dist.proba_distribution(action_logits=mean_actions)
            ...
            #print('StateDependentNoiseDistribution')
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent_vf)
        #print('evaluate actions log prob')
        #print(log_prob)
        #print(latent_pi)
        #advantage_ten = self.value_net(latent_vf).gather(1, actions[0])
        #print(advantage_ten)
...
class ActorCriticPolicy_1(BasePolicy):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
    ...
    def __init__(
        ...
        features_extractor_class == NatureCNN_Customize
        # Default network architecture, from stable-baselines
        if net_arch is None:
            if features_extractor_class == NatureCNN:
                net_arch = []
...
class ActorCriticCnnPolicy_1(ActorCriticPolicy_1):
    """
    CNN policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.

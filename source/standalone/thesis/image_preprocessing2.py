...
file_list = os.listdir("images/image3/")
...
quat = np.array([0.4984, 0.4954, 0.5058, 0.5004])
quat = quat/np.linalg.norm(quat)
rot_m = quaternion_to_matrix(quat)
# rot_m = np.linalg.inv(rot_m)
transform_arr = np.array([0.762,-0.216,0.954])
print(rot_m)
quat_optical_frame = np.array([0.500, 0.500, -0.500, 0.500])
rot_m2 = quaternion_to_matrix(quat_optical_frame)
rot_m2 = np.linalg.inv(rot_m2)
rot_m = np.matmul(rot_m,rot_m2)
i_num = 0
for _ in file_list:
    i_num += 1
    image = cv2.imread("images/image3/"+_, cv2.IMREAD_UNCHANGED)
    print(_)

...
from .push_cfg_v12 import PushEnvCfg, RandomizationCfg, YCBobjectsCfg, CameraCfg
...
class PushObservationManager(ObservationManager):
    ...
    def table_scene(self,env:PushEnv):
        # print("get observs")
        obs_ta = torch.zeros((env.num_envs,env.cfg.og_resolution.tabletop[1]+22,
                                    env.cfg.og_resolution.tabletop[0]+22,2),device=env.device)
        # obs_ta = torch.zeros((env.num_envs,env.cfg.og_resolution.tabletop[1],
        ...
        for i in range(env.num_envs):
            ...
            # observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1],
            #                         env.cfg.og_resolution.tabletop[0]])
            obs_ta[i,5:-5,5:-5,0] = torch.from_numpy(observation).to(env.device)

            im_tsdf = env.table_tsdf[i].cpu().numpy()*255
            im_tsdf_ex = np.zeros([env.cfg.og_resolution.tabletop[1]+12,
                                    env.cfg.og_resolution.tabletop[0]+12])
            im_tsdf_ex[6:-6,6:-6] = im_tsdf
            observation = np.array(im_tsdf_ex,dtype=np.uint8)
            observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1]+12,
                                    env.cfg.og_resolution.tabletop[0]+12])
            # observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1],
            #                         env.cfg.og_resolution.tabletop[0]])
            obs_ta[i,5:-5,5:-5,1] = torch.from_numpy(observation).to(env.device)

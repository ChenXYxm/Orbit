...
from .push_cfg_v17 import PushEnvCfg, RandomizationCfg, YCBobjectsCfg, CameraCfg
...
class PushEnv(IsaacEnv):
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        if not self.cfg.pre_train:
            ...
            for i in range(self.num_envs):
                vec_tmp = np.zeros(2)
                vec_tmp[0] = 0.04*np.cos(2*np.pi*self.actions[i,2].cpu().numpy())
                vec_tmp[1] = 0.04*np.sin(2*np.pi*self.actions[i,2].cpu().numpy())

...
from .push_cfg_v15 import PushEnvCfg, RandomizationCfg, YCBobjectsCfg, CameraCfg
...
from .place_new_obj_new import place_new_obj_fun,get_new_obj_contour_bbox,draw_bbox
...
class PushEnv(IsaacEnv):
    ...
    def __init__(self, cfg: PushEnvCfg = None, **kwargs):
        ...
        self.action_space = gym.spaces.MultiDiscrete([self.cfg.og_resolution.tabletop[0]+44,
                                                      self.cfg.og_resolution.tabletop[1]+44])
    ...
    def _design_scene(self) -> List[str]:
        ...
        # table
        reso = float(self.cfg.og_resolution.resolution)
        table_size = (float(self.cfg.og_resolution.tabletop[0])/reso,float(self.cfg.og_resolution.tabletop[1])/reso,0.5)
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        for i in range(self.num_envs):
            if actions[i,0]>=22 and actions[i,0]<122:
                actions[i,0] = actions[i,0]-22
            else:
                actions[i,2] = 12
            if actions[i,1]>=22 and actions[i,1]<122:
                actions[i,1] = actions[i,1]-22
            else:
                actions[i,2] = 12
        ...
        reso = float(self.cfg.og_resolution.resolution)
        action_range = (float(self.cfg.og_resolution.tabletop[0])/(reso*2.),float(self.cfg.og_resolution.tabletop[1])/(reso*2.))
    ...
    def _initialize_views(self) -> None:
        ...
        self.table_tsdf_pre = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]),device=self.device)
        self.table_tsdf_ex = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+44,
                                     self.cfg.og_resolution.tabletop[0]+44),device=self.device)
        self.table_expand_og = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+44,
                                     self.cfg.og_resolution.tabletop[0]+44),device=self.device)
        self.table_expand_og_pre = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+44,
                                     self.cfg.og_resolution.tabletop[0]+44),device=self.device)
    ...
    def get_og(self,camera):
        ...
        reso = float(self.cfg.og_resolution.resolution)
        og_boundary = (float(self.cfg.og_resolution.tabletop[0])/(reso*2.),float(self.cfg.og_resolution.tabletop[1])/(reso*2.))
        ...
        x = np.linspace(-og_boundary[0]+cam_pos[0], og_boundary[0]+cam_pos[0], Ny)
        y = np.linspace(-og_boundary[1]+cam_pos[1], og_boundary[1]+cam_pos[1], Nx)
        
        # print(np.min(pts_tab[:,0]), np.max(pts_tab[:,0]),-og_boundary[1]+cam_pos[1], og_boundary[1]+cam_pos[1])
        # print(np.min(pts_tab[:,1]), np.max(pts_tab[:,1]),-og_boundary[0]+cam_pos[0], og_boundary[0]+cam_pos[0])
        ''' origin
        ...
        index_inliers = np.argwhere((whole_point_cloud[:,1]>=-og_boundary[1]-0.11+cam_pos[1])).reshape(-1).astype(int)
        whole_point_cloud = whole_point_cloud[index_inliers]
        index_inliers = np.argwhere((whole_point_cloud[:,1]<=og_boundary[1]+0.11+cam_pos[1])).reshape(-1).astype(int)
        whole_point_cloud = whole_point_cloud[index_inliers]
        index_inliers = np.argwhere((whole_point_cloud[:,0]>=-og_boundary[0]-0.11+cam_pos[0])).reshape(-1).astype(int)
        whole_point_cloud = whole_point_cloud[index_inliers]
        index_inliers = np.argwhere((whole_point_cloud[:,0]<=og_boundary[0]+0.11+cam_pos[0])).reshape(-1).astype(int)
        whole_point_cloud = whole_point_cloud[index_inliers]
        ...
        Nx = self.cfg.og_resolution.tabletop[0]+44
        Ny = self.cfg.og_resolution.tabletop[1]+44
        # objects_pcd = o3d.geometry.PointCloud()
        # objects_pcd.points = o3d.utility.Vector3dVector(whole_point_cloud)
        # o3d.visualization.draw_geometries([objects_pcd])
        ################ original table
        occupancy_ex = np.zeros( (Ny,Nx) )
        occupancy_ex[22:self.cfg.og_resolution.tabletop[1]+22,22:self.cfg.og_resolution.tabletop[0]+22] = 1
        ...
        # plt.show()
        ################################
        Tsdf_ex = occupancy_ex.copy()
        ###################### get the point cloud of objs
        point_obj = o3d.geometry.PointCloud()
        pts_tmp = pts_ex.copy()
        pts_tmp[:,2] = 0
        point_obj.points = o3d.utility.Vector3dVector(pts_tmp)
        x = np.linspace(-og_boundary[0]+cam_pos[0]-0.11, og_boundary[0]+cam_pos[0]+0.11, Ny)
        y = np.linspace(-og_boundary[1]+cam_pos[1]-0.11, og_boundary[1]+cam_pos[1]+0.11, Nx)
        ''' origin
        x = np.linspace(np.min(pts_tab[:,0]), np.max(pts_tab[:,0]), Nx)
        y = np.linspace(np.min(pts_tab[:,1]), np.max(pts_tab[:,1]), Ny)
        '''
        ...
        dist = np.array(distance)
        # norm = mplc.Normalize(vmin=min(distance), vmax=max(distance), clip=True)
        Tsdf_ex = dist.reshape(Ny,Nx)
        Tsdf_ex = np.fliplr(Tsdf_ex)
        # fig, (ax1,ax2) = plt.subplots(1, 2, figsize=(15, 10))
        # ax1.imshow(Tsdf)
        # ax2.imshow(Tsdf_ex)
        # plt.show()
        # print(Tsdf_ex.shape,Tsdf.shape)
        return occupancy,occupancy_ex,Tsdf,Tsdf_ex
    ...
    def _update_table_og(self):
        if self.reset_f:
            for i in range(self.num_envs):
                ...
                og,og_ex,tsdf,tsdf_ex = self.get_og(self.cams[i])
                # fig, (ax1,ax2) = plt.subplots(1, 2, figsize=(15, 10))
                # ax1.imshow(og)
                # ax2.imshow(tsdf)
                # plt.show()
                ...
                self.table_tsdf_ex[i] = torch.from_numpy(tsdf_ex.copy()).to(self.device)
            ...
            if self.actions_origin[i,2]<9:
                ...
                if torch.sum(table_og_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y])==0:
                    if self.actions_origin[i,2] == 0:
                        start_ind_y = max(self.actions_origin[i][1]-9,0)
                        end_ind_y = min(self.actions_origin[i][1],self.cfg.og_resolution.tabletop[1])
                        if torch.sum(table_og_tmp[self.actions_origin[i][0],start_ind_y:end_ind_y])>=1:
                            self.check_reaching[i] = 1 
                    elif self.actions_origin[i,2] == 2:
                        start_ind_x = max(self.actions_origin[i][0],0)
                        end_ind_x = min(self.actions_origin[i][0]+9,self.cfg.og_resolution.tabletop[0])
                        ...
                        end_ind_y = min(self.actions_origin[i][1]+9,self.cfg.og_resolution.tabletop[1])
                        if torch.sum(table_og_tmp[self.actions_origin[i][0],start_ind_y:end_ind_y])>=1:
                            self.check_reaching[i] = 1 
                    elif self.actions_origin[i,2] == 6:
                        start_ind_x = max(self.actions_origin[i][0]-9,0)
...
class PushObservationManager(ObservationManager):
    ...
    def table_scene(self,env:PushEnv):
        # print("get observs")
        obs_ta = torch.zeros((env.num_envs,env.cfg.og_resolution.tabletop[1]+44,
                                    env.cfg.og_resolution.tabletop[0]+44,2),device=env.device)
        # obs_ta = torch.zeros((env.num_envs,env.cfg.og_resolution.tabletop[1],
        ...
        for i in range(env.num_envs):
            ...
            # plt.show()
            observation = np.array(im,dtype=np.uint8)
            observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1]+44,
                                    env.cfg.og_resolution.tabletop[0]+44])
            # observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1],
            #                         env.cfg.og_resolution.tabletop[0]])
            obs_ta[i,:,:,0] = torch.from_numpy(observation).to(env.device)
        ...
        for i in range(env.num_envs):
            if env.actions_origin[i,2]<9:
                ...
                if (env.actions_origin[i,2]%4) == 0:
                    ...
                    end_ind_x = min(env.actions_origin[i][0]+4,env.cfg.og_resolution.tabletop[0])
                    ...
                    end_ind_x = min(env.actions_origin[i][0]+2,env.cfg.og_resolution.tabletop[0])
                    start_ind_y = max(env.actions_origin[i][1]-2,0)
                    end_ind_y = min(env.actions_origin[i][1]+4,env.cfg.og_resolution.tabletop[1])
                print('reward'+str(i))
                print(start_ind_x,end_ind_x,start_ind_y,end_ind_y)
                if torch.sum(table_og_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y])>0:
                    reward_near[i] = -2
                    ...
                    # print('no overlap')
                    if env.actions_origin[i,2] == 0:
                        start_ind_y = max(env.actions_origin[i][1]-9,0)
                        end_ind_y = min(env.actions_origin[i][1],env.cfg.og_resolution.tabletop[1])
                        if torch.sum(table_og_tmp[env.actions_origin[i][0],start_ind_y:end_ind_y]) == 0:
                            reward_near[i] = -2
                    elif env.actions_origin[i,2] == 2:
                        start_ind_x = max(env.actions_origin[i][0],0)
                        end_ind_x = min(env.actions_origin[i][0]+9,env.cfg.og_resolution.tabletop[0])
                        if torch.sum(table_og_tmp[start_ind_x:end_ind_x,env.actions_origin[i][1]]) == 0:
                            reward_near[i] = -2
                    elif env.actions_origin[i,2] == 4:
                        start_ind_y = max(env.actions_origin[i][1],0)
                        end_ind_y = min(env.actions_origin[i][1]+9,env.cfg.og_resolution.tabletop[1])
                        if torch.sum(table_og_tmp[env.actions_origin[i][0],start_ind_y:end_ind_y]) == 0:
                            reward_near[i] = -2
                    elif env.actions_origin[i,2] == 6:
                        start_ind_x = max(env.actions_origin[i][0]-9,0)
                        end_ind_x = min(env.actions_origin[i][0],env.cfg.og_resolution.tabletop[0])
                        if torch.sum(table_og_tmp[start_ind_x:end_ind_x,env.actions_origin[i][1]]) == 0:
                            reward_near[i] = -2
                ...
                if env.actions_origin[i,2] == 0:
                    start_ind_y = max(env.actions_origin[i][1]-10,0)
                    end_ind_y = min(env.actions_origin[i][1],env.cfg.og_resolution.tabletop[1])
                elif env.actions_origin[i,2] == 2:
                    start_ind_x = max(env.actions_origin[i][0],0)
                    end_ind_x = min(env.actions_origin[i][0]+10,env.cfg.og_resolution.tabletop[0])
                elif env.actions_origin[i,2] == 4:
                    start_ind_y = max(env.actions_origin[i][1],0)
                    end_ind_y = min(env.actions_origin[i][1]+10,env.cfg.og_resolution.tabletop[1])
                elif env.actions_origin[i,2] == 6:
                    start_ind_x = max(env.actions_origin[i][0]-10,0)
                    end_ind_x = min(env.actions_origin[i][0],env.cfg.og_resolution.tabletop[0])

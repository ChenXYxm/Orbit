...
from .push_cfg_v7 import PushEnvCfg, RandomizationCfg, YCBobjectsCfg, CameraCfg
...
class PushEnv(IsaacEnv):
    ...
    def _reset_idx(self, env_ids: VecEnvIndices):
        # randomize the MDP
        # -- robot DOF state
        self.total_step = self.total_step+self.step_num
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        if not self.cfg.pre_train:
            ...
            self._update_table_og()
        for i in range(self.num_envs):
            if self.check_reaching[i] ==0 and self.step_num<2 :
                self.place_success[i] = 0
            
            elif self.place_success[i] >=0.5 and self.step_num<2:
                print('wrong cases')
                self.wrong_cases+=1

...
from .push_cfg_v16 import PushEnvCfg, RandomizationCfg, YCBobjectsCfg, CameraCfg
...
class PushEnv(IsaacEnv):
    ...
    def __init__(self, cfg: PushEnvCfg = None, **kwargs):
        ...
        for i in range(18):
            ...
            obj_cfg1.meta_info = RigidObjectCfg.MetaInfoCfg(usd_path=ycb_usd_paths[ycb_name[0]],scale=(0.42,0.28,1.35),)
            obj_cfg1.init_state = RigidObjectCfg.InitialStateCfg(
            pos=(2-0.1*i, 0.8, -0.4), rot=(1.0, 0.0, 0.0, 0.0), lin_vel=(0.0, 0.0, 0.0), ang_vel=(0.0, 0.0, 0.0)
            )
    ...
    def reward_reaching(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
            ...
            if reward_near[i]==0 and env.stop_pushing[i]==0:
                if env.actions_origin[i][0] >= env.cfg.og_resolution.tabletop[0]-3 or env.actions_origin[i][0]<=3:
                    # print(env.actions_origin[i])
                    reward_near[i] = -2
                elif env.actions_origin[i][1] >= env.cfg.og_resolution.tabletop[0]-3 or env.actions_origin[i][1]<=3:
                    # print(env.actions_origin[i])
                    reward_near[i] = -2 

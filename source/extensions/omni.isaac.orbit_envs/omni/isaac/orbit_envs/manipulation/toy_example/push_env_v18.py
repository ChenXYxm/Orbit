...
from .push_cfg_v18 import PushEnvCfg, RandomizationCfg, YCBobjectsCfg, CameraCfg
...
class PushEnv(IsaacEnv):
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        if not self.cfg.pre_train:
            ...
            for i in range(self.num_envs):
                vec_tmp = np.zeros(2)
                vec_tmp[0] = 0.025*np.cos(2*np.pi*self.actions[i,2].cpu().numpy())
                vec_tmp[1] = 0.025*np.sin(2*np.pi*self.actions[i,2].cpu().numpy())
                # vec_tmp[0] = 0.1*np.cos(2*np.pi*self.actions[i,2].cpu().numpy())
                # vec_tmp[1] = 0.1*np.sin(2*np.pi*self.actions[i,2].cpu().numpy())
                actions_tmp[i,:2] = actions_tmp[i,:2] - torch.from_numpy(vec_tmp).to(self.device)
            for i in range(2):
    ...
    def _randomize_table_scene(self,env_ids: torch.Tensor):
        ...
        # print('new obj type',self.new_obj_type)
        fileObject2.close()
        # print(env_ids)

    ...
    def _check_reaching_toy_v2(self):
        ...
        for i in range(self.num_envs):
            ...
            if (self.actions_origin[i][0] >=start_t and 
                ...
                table_og_tmp1 = table_og_tmp.clone().detach()
...
class PushObservationManager(ObservationManager):
    ...
    def table_scene(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
            ...
            im_tsdf = env.table_tsdf[i].cpu().numpy()/float(np.max(env.table_tsdf[i].cpu().numpy()))*255
...
class PushRewardManager(RewardManager):
    ...
    def reward_og_change(self,env:PushEnv):
        ...
        print('reach',env.check_reaching)

        for i in range(env.num_envs):
            if env.check_reaching[i]>0:
                delta_tmp = env.table_og[i].clone() - env.table_og_pre[i].clone()
                # fig, (ax1,ax2) = plt.subplots(1, 2, figsize=(7, 4))
                # ax1.imshow(env.table_og[i].cpu().numpy())
                # ax2.imshow(env.table_og_pre[i].cpu().numpy())
                # plt.show()
                if torch.sum(torch.abs(delta_tmp))>=12:
                    delta_og[i] = 1

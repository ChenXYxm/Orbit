...
class PushEnv(IsaacEnv):
    ...
    def __init__(self, cfg: PushEnvCfg = None, **kwargs):
        ...
        for i in range(16):
            ...
            obj_cfg4.meta_info = RigidObjectCfg.MetaInfoCfg(usd_path=ycb_usd_paths[ycb_name[3]],scale=(0.42,0.28,1.05),)
        ...
        for i in range(10):
            ...
            obj_cfg5.meta_info = RigidObjectCfg.MetaInfoCfg(usd_path=ycb_usd_paths[ycb_name[4]],scale=(0.5,1,1.2),)
            obj_cfg5.init_state = RigidObjectCfg.InitialStateCfg(
            pos=(2-0.25*i, 1.5, -0.4), rot=(1.0, 0.0, 0.0, 0.0), lin_vel=(0.0, 0.0, 0.0), ang_vel=(0.0, 0.0, 0.0)
            )
        ...
        self.obj6 = []
        for i in range(10):
            obj_cfg6 = RigidObjectCfg()
            obj_cfg6.meta_info = RigidObjectCfg.MetaInfoCfg(usd_path=ycb_usd_paths[ycb_name[5]],scale=(0.5,0.6,1.2),)
            obj_cfg6.init_state = RigidObjectCfg.InitialStateCfg(
            pos=(2-0.25*i, 1.7, -0.4), rot=(1.0, 0.0, 0.0, 0.0), lin_vel=(0.0, 0.0, 0.0), ang_vel=(0.0, 0.0, 0.0)
            )
            obj_cfg6.rigid_props = RigidObjectCfg.RigidBodyPropertiesCfg(
                solver_position_iteration_count=16,
                solver_velocity_iteration_count=1,
                max_angular_velocity=0.5,
                max_linear_velocity=0.5,
            ...
            obj_cfg6.physics_material = RigidObjectCfg.PhysicsMaterialCfg(
                static_friction=0.5, dynamic_friction=0.5, restitution=0.0, prim_path="/World/Materials/cubeMaterial"
            )
            self.obj6.append(RigidObject(obj_cfg6))
            self.obj_name_list.append(ycb_name[5]+str(i))
        ...
        for i,obj_t in enumerate(self.obj5):
            obj_t.update_buffers(self.dt)
        for i,obj_t in enumerate(self.obj6):
            obj_t.update_buffers(self.dt)
    ...
    def _design_scene(self) -> List[str]:
        ...
        for i,obj_t in enumerate(self.obj6):
            obj_t.spawn(self.template_env_ns + f"/Objs/obj6/obj_{i}")
        # camera
    ...
    def _reset_idx(self, env_ids: VecEnvIndices):
        ...
        self.table_og_pre[env_ids_list] = self.table_og[env_ids_list].copy()
        self.table_expand_og_pre[env_ids_list] = self.table_expand_og[env_ids_list].copy()
        self.table_tsdf_pre[env_ids_list] = self.table_tsdf[env_ids_list].copy()
        # self.falling_obj[env_ids] = 0
        self.falling_obj_all[env_ids_list] = 0
    ...
    def _get_obj_mask(self,env_ids: VecEnvIndices):
        ...
        mask[:,s_x_ind:e_x_ind,s_y_ind:e_y_ind] = self.new_obj_mask.copy()
        for j in env_ids.tolist():
            # print("mask j")
            # print(env_ids[j])
            self.obj_masks[j] = mask[j].copy()
            # plt.imshow(self.new_obj_mask[j])
            # plt.draw()
            # print('mask after get_obj_mask')
            # plt.imshow(mask[j])
            # plt.show() 
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        self.table_tsdf_pre = self.table_tsdf.copy()
        self.table_expand_og_pre = self.table_expand_og.copy()
        ...
        # actions_tmp[:,1] +=0.1
        ''' modified for toy example'''
        # print(self.cfg.pre_train,self.check_reaching)
        ...
        for i in range(self.num_envs):
            if self.try_placing[i]>0.5:
                ...
                self.actions_origin_2[i][1]<self.cfg.og_resolution.tabletop[0]+start_t and self.actions_origin_2[i][2]>=8):
                    ...
                    # print('min x',min(ind_x),'max x',max(ind_x))
                    mask_tmp_t = rotate_45(dilated_matrix,-action_tmp[2]*30)
                    # plt.imshow(mask_tmp_t)
                    # plt.show()
                    # print(rotation)
                    ...
                    if np.max(occu_tmp) == 1:
                        ...
                        if np.max(occu_tmp) < 0.5:
                            ...
                            if obj_name_i in ["mug","tomatoSoupCan","pitcherBase","tunaFishCan","bowl","banana"]:
                                rot = convert_quat(tf.Rotation.from_euler("XYZ", (-90,-action_tmp[2]*30,0), degrees=True).as_quat(), to="wxyz")
                            else:
                                rot = convert_quat(tf.Rotation.from_euler("XYZ", (0,0,-(-action_tmp[2]*30)), degrees=True).as_quat(), to="wxyz")
                            env_ids_tmp = torch.from_numpy(np.array([i])).to(self.device)
    ...
    def _initialize_views(self) -> None:
        ...
        for i,obj_t in enumerate(self.obj6):
            obj_t.initialize(self.env_ns + "/.*"+ f"/Objs/obj6/obj_{i}")
        ...
        #                              self.cfg.og_resolution.tabletop[0]),device=self.device)
        self.table_og = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]))
        self.table_og_pre = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]))
        self.table_tsdf = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]))
        self.table_tsdf_pre = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]))
        self.table_expand_og = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+12,
                                     self.cfg.og_resolution.tabletop[0]+12))
        self.table_expand_og_pre = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+12,
                                     self.cfg.og_resolution.tabletop[0]+12))
        self.obj_masks = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]))
        self.max_tsdf = torch.zeros((self.num_envs,),device=self.device)
        self.place_success = torch.zeros((self.num_envs,),device=self.device)
        self.falling_obj = torch.zeros((self.num_envs,),device=self.device)
        self.falling_obj_all = torch.zeros((self.num_envs,),device=self.device)
        self.step_count = torch.zeros((self.num_envs,),device=self.device)
        ...
        self.done_obs = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]))
        self.random_patches = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]))
    def get_pcd(self,camera):
        ...
        # print('color of point cloud')
        # print(pointcloud_rgb)
    ...
    def get_og(self,camera):
        ...
        # fig, (ax1,ax2,ax3) = plt.subplots(1, 3, figsize=(15, 10))
        # ax1.imshow(occupancy_ex_ori)
        # ax2.imshow(occupancy_ex_blur)
        # ax3.imshow(occupancy_ex)
        # plt.show()
        return occupancy_blur,occupancy_ex,Tsdf
    def tabletop_scene(self):
        # print("get observs")
        ...
        obs_ta = torch.zeros((self.num_envs,self.cfg.og_resolution.expand_obs[1],
                                    self.cfg.og_resolution.expand_obs[0],2),device=self.device)
        for i in range(self.num_envs):
            
            im = self.table_expand_og[i].copy()*255/2.0
            ...
            observation = observation[:,np.newaxis].reshape([self.cfg.og_resolution.tabletop[1]+12,
                                    self.cfg.og_resolution.tabletop[0]+12])
            obs_ta[i,start_t-6:self.cfg.og_resolution.expand_obs[1]-start_t+6,
                   start_t-6:self.cfg.og_resolution.expand_obs[1]-start_t+6,0] = torch.from_numpy(observation).to(self.device)

            im_tsdf = self.table_tsdf[i].copy()/float(np.max(self.table_tsdf[i].copy()))*255
            observation = np.array(im_tsdf,dtype=np.uint8)
            # observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1]+12,
            #                         env.cfg.og_resolution.tabletop[0]+12])
            observation = observation[:,np.newaxis].reshape([self.cfg.og_resolution.tabletop[1],
                                    self.cfg.og_resolution.tabletop[0]])
            obs_ta[i,start_t:self.cfg.og_resolution.expand_obs[1]-start_t,
                   start_t:self.cfg.og_resolution.expand_obs[1]-start_t,1] = torch.from_numpy(observation).to(self.device)
    ...
    def _get_obj_info(self,env_ids: VecEnvIndices):
        ...
        for i in env_ids.tolist():
            mask_obj = self.obj_masks[i].copy()
    ...
    def _place_obj(self,pos,rot,env_ids:VecEnvIndices,obj_type:str):
        for k in env_ids.tolist():
            if k in self.obj_on_table:
                ...
                elif obj_type == self.cfg.YCBdata.ycb_name[5]:
                    root_state = self.obj6[_].get_default_root_state(env_ids_tmp)
                    root_state[:,0:3] = torch.from_numpy(np.array(pos)).to(self.device)
                    root_state[:, 3:7] = torch.from_numpy(np.array(rot)).to(self.device)
                    root_state[:, 0:3] += self.envs_positions[env_ids_tmp]
    ...
    def _update_table_og(self):
        if self.reset_f:
            for i in range(self.num_envs):
                ...
                # ax2.imshow(tsdf)
                # plt.show()
                self.table_og[i] = og.copy()
                self.table_expand_og[i] = og_ex.copy()
                self.table_tsdf[i] = tsdf.copy()
    ...
    def _check_reaching_toy_v2(self):
        ...
        start_t = int((self.cfg.og_resolution.expand_obs[0]-self.cfg.og_resolution.tabletop[0])/2)
        for i in range(self.num_envs):
            table_og_tmp = self.table_og_pre[i]
            ...
            if (self.actions_origin[i][0] >=start_t and 
                ...
                '''
                    ...
                    # theta = -(np.pi/4.0)*float(self.actions_origin[i,2]%4)
                    theta = -np.radians(float(self.actions_origin[i,2])*(360/8))
                    rotation_matrix = np.array([[np.cos(theta), -np.sin(theta)],
                        [np.sin(theta), np.cos(theta)]])
                    # rotation_matrix = np.array([[np.cos(theta), np.sin(theta)],
                    #         [-np.sin(theta), np.cos(theta)]])
                    # Original coordinates of the target value
                    
                    original_coords = np.array([[self.actions_origin[i][0].cpu()], [self.actions_origin[i][1].cpu()]])
                    # print('action origin: ', self.actions_origin[i])
                    # original_coords = original_coords+start_t-int(self.cfg.og_resolution.expand_obs[1]/2)+1
                    original_coords = original_coords+start_t-int(self.cfg.og_resolution.expand_obs[1]/2)+0.5
                    # print('old : ', original_coords)
                    # Apply rotation to get new coordinates
                    new_coords = np.dot(rotation_matrix, original_coords)
                    # # Round to nearest integer to get new index
                    new_x = int(round(new_coords[0, 0]+self.cfg.og_resolution.expand_obs[1]/2)-0.5)
                    new_y = int(round(new_coords[1, 0]+self.cfg.og_resolution.expand_obs[1]/2)-0.5)
                    # print('old : ', original_coords,'new: ',new_x, new_y)
                    
                    table_og_tmp = self.table_og_pre[i].clone()
                    table_og_tmp1 = self.table_og_pre[i].clone()
                    # table_og_tmp1[self.actions_origin[i][0].cpu(),self.actions_origin[i][1].cpu()] = 2
                    table_ex_tmp = torch.zeros((1,1,self.cfg.og_resolution.expand_obs[1],self.cfg.og_resolution.expand_obs[1])).cuda()
                    table_ex_tmp[0,0,start_t:self.cfg.og_resolution.tabletop[1]+start_t,start_t:self.cfg.og_resolution.tabletop[1]+start_t] = table_og_tmp1.clone().detach()
                    affine_mat_before = np.asarray([[np.cos(-theta), np.sin(-theta), 0],[-np.sin(-theta), np.cos(-theta), 0]])
                    affine_mat_before.shape = (2,3,1)
                    affine_mat_before = torch.from_numpy(affine_mat_before).permute(2,0,1).float()
                    flow_grid_before = F.affine_grid(Variable(affine_mat_before, requires_grad=False).cuda(), table_ex_tmp.size(),align_corners=False)
                    rotate_table_ex_tmp = F.grid_sample(Variable(table_ex_tmp, requires_grad=False).cuda(), flow_grid_before, mode='nearest',align_corners=False)
                    obs_space = rotate_table_ex_tmp[0,0].clone().detach().cpu().numpy()
                    # ind_action = np.where(obs_space==2)
                    # print("ind_action rot",ind_action )
                    # obs_space[new_x,new_y] = 2
                    # fig, (ax1,ax2) = plt.subplots(1,2, figsize=(7, 4))
                    # ax1.imshow(np.array(obs_space))
                    # ax2.imshow(np.array(table_og_tmp1.clone().detach().cpu()))
                    # plt.show()
                    # new_x = int(ind_action[0][0])
                    # new_y = int(ind_action[1][0])
                    # print('old : ', original_coords,'new: ',new_x, new_y)
                    start_ind_x = max(new_x-1,0)
                    end_ind_x = min(new_x+2,self.cfg.og_resolution.expand_obs[1])
                    start_ind_y = max(new_y-1,0)
                    end_ind_y = min(new_y+2,self.cfg.og_resolution.expand_obs[1])
                    obs_space_tmp = obs_space.copy()
                    # print(np.sum(obs_space[start_ind_x:end_ind_x,start_ind_y:end_ind_y]))
                    if np.sum(obs_space[start_ind_x:end_ind_x,start_ind_y:end_ind_y])==0:
                        obs_space_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y] = 2
                        # print(start_ind_x,end_ind_x,start_ind_y,end_ind_y)
                        # print(new_x,new_y)
                        start_ind_y = max(new_y-5,0)
                        end_ind_y = min(new_y,self.cfg.og_resolution.expand_obs[1])
                        if np.sum(obs_space[new_x,start_ind_y:end_ind_y])>=1:
                            self.check_reaching[i] = 1
                        # print('pushing')
                        # print(np.sum(obs_space[new_x,start_ind_y:end_ind_y]))
                        # print(start_ind_x,end_ind_x,start_ind_y,end_ind_y)
                    #     obs_space_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y] = 3
                    #     # print('start point not overlap')
                    # else:
                    #     obs_space_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y] = 2
                    #     start_ind_y = max(new_y-5,0)
                    #     end_ind_y = min(new_y,self.cfg.og_resolution.expand_obs[1])
                    #     obs_space_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y] = 3
                    #     print('start point overlap')
                    # start_ind_x = max(new_x-1,0)
                    # end_ind_x = min(new_x+2,self.cfg.og_resolution.expand_obs[1])
                    # start_ind_y = max(new_y-1,0)
                    # end_ind_y = min(new_y+2,self.cfg.og_resolution.expand_obs[1])
                    # table_og_tmp1[self.actions_origin[i][0].cpu(),self.actions_origin[i][1].cpu()] = 2
                    # print(start_ind_x,end_ind_x,start_ind_y,end_ind_y)
                    # obs_space[start_ind_x,start_ind_y] = 2
                    # obs_space[end_ind_x,end_ind_y] = 2
                    # fig, (ax1,ax2,ax3) = plt.subplots(1,3, figsize=(7, 4))
                    # ax1.imshow(np.array(obs_space))
                    # ax3.imshow(np.array(obs_space_tmp))
                    # ax2.imshow(np.array(table_og_tmp1.clone().detach().cpu()))
                    # plt.show()
                    del obs_space, flow_grid_before,rotate_table_ex_tmp,table_og_tmp,table_og_tmp1
                '''
            ...
            # print('check reaching',self.check_reaching)
    ...
    def _randomize_table_scene(self,env_ids: torch.Tensor):
        ...
        for i in env_ids.tolist():
            ...
            # print('mask change after randomize table')
...
class PushObservationManager(ObservationManager):
    ...
    def table_scene(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
            ...
            im = env.table_expand_og[i].copy()*255/2.0
            # im = env.table_og[i].cpu().numpy()*255
            ...
            im_tsdf = env.table_tsdf[i].copy()/float(np.max(env.table_tsdf[i].copy()))*255
            observation = np.array(im_tsdf,dtype=np.uint8)
            # observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1]+12,
            #                         env.cfg.og_resolution.tabletop[0]+12])
            observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1],
...
class PushRewardManager(RewardManager):
    ...
    def penaltizing_pushing_outside(self,env:PushEnv):
        ...
        env_tab_ex_tmp = env.table_expand_og.copy()
        env_tab_ex_tmp_pre = env.table_expand_og_pre.copy()
        for i in range(env.num_envs):
            # fig, (ax1,ax2) = plt.subplots(1, 2, figsize=(7, 4))
            # ax1.imshow(env_tab_ex_tmp[i].cpu().numpy())
            # ax2.imshow(env_tab_ex_tmp_pre[i].cpu().numpy())
    ...
    def penaltizing_falling(self,env:PushEnv):
        fallen = torch.zeros((env.num_envs,),device=self.device)
        for i in range(env.num_envs):
            if env.falling_obj[i]>0.5:
                fallen[i] = -1
        ...
        # print('fallen')
        # print(fallen)
        return fallen.type(torch.float16)
    def check_placing(self,env:PushEnv):
        print('check placing reward manager',env.place_success)
        placed = torch.zeros((env.num_envs,),device=self.device)
        for i in range(env.num_envs):
            # if env.place_success[i]>=0.5 and env.check_reaching[i]>=0.5:
                
            #     if env.falling_obj[i] == 0 and env.pixel_outside_table[i] == 0:
            #         placed[i] = 1
            if env.place_success[i]>=0.5:
                 placed[i] = 1
    ...
    #     for i in range(env.num_envs):
    #         table_og_tmp = env.table_og_pre[i].copy()
    #         if env.actions_origin[i][1]>=3:
    #             if np.sum(table_og_tmp[env.actions_origin[i][0],env.actions_origin[i][1]-3:env.actions_origin[i][1]])>=1:
    #                 reward_near[i] = 1
    ...
    #                         if np.sum(table_og_tmp[env.actions_origin[i][0]-1:env.actions_origin[i][0]+2,env.actions_origin[i][1]-1:env.actions_origin[i][1]+2])==0:
    #                             reward_near[i] += 0.5
    ...
    def reward_distribution_closer(self,env:PushEnv):
        delta_og = torch.zeros((env.num_envs,),device=self.device)
        for i in range(env.num_envs):
            ind_pre = np.where(env.table_og_pre[i]>=0.8)
            ind_cur = np.where(env.table_og[i]>=0.8)
    ...
    #         if np.sum(table_og_tmp[env.actions_origin[i][0],start_ind_y:end_ind_y])>=1:
    ...
    #                     if np.sum(table_og_tmp[j,start_ind_y:end_ind_y])>0:
    #                         # print('pushing reward')
    #                         reward_near[i] = -3
    #                         # print(reward_near)
    #                         ################# modified in Dec 10
    #                         ind_pre = np.where(env.table_og_pre[i]<=0.5)
    #                         ind_pre_x = ind_pre[0].clone()
    #                         ind_pre_y = ind_pre[1].clone()
    #                         ind_tmp = np.where(ind_pre_y>=int(start_ind_y))
    #                         ind_tmp = ind_tmp[0]
    ...
    #                             ind_tmp = np.where(ind_pre_x==int(j))

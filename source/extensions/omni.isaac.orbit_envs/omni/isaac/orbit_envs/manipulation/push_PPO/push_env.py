...
class PushEnv(IsaacEnv):
    ...
    def _initialize_views(self) -> None:
        ...
        self.table_og_pre = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]),device=self.device)
        self.height_map = np.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
    ...
    def get_og(self,camera):
        ...
        count_num = np.zeros((Nx,Ny))
        for i in range(len(u)):
            height_map[v[i],u[i]] += z[i]
            count_num[v[i],u[i]] += 1
        count_num[np.where(count_num==0)] = 1
        for i in range(Nx):
            for j in range(Ny):
                height_map[i,j] = height_map[i,j]/count_num[i,j]
        height_map = np.fliplr(height_map)
        # print('max',np.max(height_map))
        ...
        return occupancy,occupancy_ex,Tsdf,height_map
    ...
    def _update_table_og(self):
        if self.reset_f:
            for i in range(self.num_envs):
                ...
                og,og_ex,tsdf,height_map= self.get_og(self.cams[i])
                # fig, (ax1,ax2) = plt.subplots(1, 2, figsize=(15, 10))
                # ax1.imshow(og)
                # ax2.imshow(tsdf)
                # plt.show()
                self.table_og[i] = torch.from_numpy(og.copy()).to(self.device)
                self.height_map[i] = height_map.copy()
    ...
    def _check_reaching_toy_v2(self):
        ...
        for i in range(self.num_envs):
            ...
            image_tmp = table_og_tmp.clone().detach().cpu().numpy()
            image_tmp_ori = image_tmp.copy()
            ...
            if torch.sum(table_og_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y])==0:
                if self.actions_origin[i,2] == 0:
                    ...
                    if torch.sum(table_og_tmp[self.actions_origin[i][0],start_ind_y:end_ind_y])>=1:
                        self.check_reaching[i] = 1 
                    image_tmp[self.actions_origin[i][0],start_ind_y:end_ind_y]=3
                    ...
                    if torch.sum(table_og_tmp[start_ind_x:end_ind_x,self.actions_origin[i][1]])>=1:
                        self.check_reaching[i] = 1 
                    image_tmp[start_ind_x:end_ind_x,self.actions_origin[i][1]]=3
...
class PushObservationManager(ObservationManager):
    ...
    def table_scene(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
            ...
            # print(np.max(np.squeeze(obs_ta[i,:,:,1].cpu().numpy())))
            # print(np.max(np.squeeze(obs_ta[i,:,:,0].cpu().numpy())))
            # print(np.min(np.squeeze(obs_ta[i,:,:,1].cpu().numpy())))
            # print(np.min(np.squeeze(obs_ta[i,:,:,0].cpu().numpy())))
            # fig, (ax1,ax2) = plt.subplots(1, 2, figsize=(15, 10))

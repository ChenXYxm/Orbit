...
from .place_new_obj import place_new_obj_fun,get_new_obj_contour_bbox,placing_compare_fun,placing_compare_fun_2
...
class PushEnv(IsaacEnv):
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        for i in range(self.num_envs):
            if self.actions[i,2]>1.0 and self.place_success[i]>0.5:
                # print('stop action')
        ...
        if not self.cfg.pre_train:
            for i,value_timeout in enumerate(self.extras['time_outs']):
                if not value_timeout:
                    if self.place_success[i]>=0.5:
                        self.extras["time_outs"][i] = True
    ...
    def _check_placing(self):
        ...
        for i in range(self.num_envs):
            if self.place_success[i]==0 and self.check_reaching[i]>0.5:
                ...
                # print('new obj vertices',vertices_new_obj)  
                # plt.imshow(occu_tmp)
                # plt.show()
                flag_found = False
                if flag_found:
                    ...
                    flag_found,new_obj_pos = placing_compare_fun_2(occupancy,self.obj_masks[i])
                    if flag_found:
                        self.place_success[i] = 1
                        self.place_success_all +=1.0
                    # print('mask shape after placing check')
                    # plt.imshow(self.obj_masks[i])
                    # plt.show()
                    # print('place')
                    # print(env_ids_tmp)
                    # print(self.place_success_all)
    ...
    def _check_termination(self) -> None:
        ...
        if self.cfg.terminations.episode_timeout:
            ...
            self.reset_buf = torch.where(self.place_success >= 0.5, 1, self.reset_buf)
        print('terminated',self.reset_buf,'time out',self.episode_length_buf,'place success',self.place_success)
    ...
    def _randomize_table_scene(self,env_ids: torch.Tensor):
        ...
        # env_path = "new_table/"+file_name[choosen_env_id]
...
class PushRewardManager(RewardManager):
                ...
                if env.falling_obj[i]==0 and env.pixel_outside_table[i]==0:
                    ...
                    if not env.cfg.pre_train:
                        if max_curr > max_pre and not env.cfg.pre_train:
                                ...
                                max_tsdf_increase[i] = 1

                        '''new
                        result1 = placing_compare_fun(env.table_og_pre[i],env.obj_masks[i])
                        result2 = placing_compare_fun(env.table_og[i],env.obj_masks[i])
                        if np.min(result2)<np.min(result1):
    ...
    def penaltizing_pushing_outside(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
            ...
            # plt.show()
            if not env.cfg.pre_train:
                if np.sum(env_tab_ex_tmp[i])-np.sum(env_tab_ex_tmp_pre[i])<=10:
                    if env.check_reaching[i]>0.5:
    ...
    def check_placing(self,env:PushEnv):
        ...
        # print('placing',placed)
        # print(env.place_success)
        # print(env.place_success)
        return placed.type(torch.float16)

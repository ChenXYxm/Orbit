...
def placing_compare_fun(occu_ori,mask_ori):
    # mask = np.zeros((40,40))
    ...
    for i in range(1):
        ...
        # ax3.imshow(result)
        # plt.show()
        # print('con result',np.min(result),'con max',np.max(result),'mask max',np.max(mask_tmp))
...
def place_new_obj_fun(occu_ori,new_obj):
    ...
    if len(polygons)>=1:
        ...
        for i in ori_polygons:
            dila_polygons.append(i.buffer(2.6))

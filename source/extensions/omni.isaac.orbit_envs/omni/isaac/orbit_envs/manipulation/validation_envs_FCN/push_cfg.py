        ...
        for i in range(len(file_list)):
            for j in range(len(obj_name)):
                if obj_name[j] in file_list[i]:
                    ...
                    # if obj_name[j] == 'Cube3':
                        # plt.imshow(self.mask[obj_name[j]])
                        # plt.show()
                    fileObject2.close()
...
class PushEnvCfg(IsaacEnvCfg):
    ...
    # General Settings
    env: EnvCfg = EnvCfg(num_envs=4096, env_spacing=2.5, episode_length_s=3.00)
    viewer: ViewerCfg = ViewerCfg(debug_vis=False, eye=(0.1, 5.5, 5.5), lookat=(0.0, 0.0, 0.0))
    # Physics settings
    sim: SimCfg = SimCfg(
        dt=0.01,
        substeps=1,

...
class PushEnv(IsaacEnv):
    ...
    def __init__(self, cfg: PushEnvCfg = None, **kwargs):
        ...
        self.action_space = gym.spaces.MultiDiscrete([self.cfg.og_resolution.tabletop[0],
                                                      self.cfg.og_resolution.tabletop[1],4])
    ...
    def _design_scene(self) -> List[str]:
        ...
        # table
        table_size = (float(self.cfg.og_resolution.tabletop[0])/(100.0*self.cfg.og_resolution.scale),
                      float(self.cfg.og_resolution.tabletop[1])/(100.0*self.cfg.og_resolution.scale),0.5)
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        if self.actions_origin[0,2]>100:
            self.actions_origin[:,:] = 0
        ...
        self.actions[:,2] = self.actions[:,2]/4.0
        ...
        ################# stop pushing
        for i in range(self.num_envs):
            if self.check_reaching[i] == 0 or self.actions[0,2]>2:
                        # print("stop actions")
                        # print(self.actions)
    ...
    def _initialize_views(self) -> None:
        ...
        # time-step = 0
        self.enlarge_pixel = int(12*self.cfg.og_resolution.scale)
        ...
        self.table_tsdf_pre = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]),device=self.device)
        self.table_expand_og = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+self.enlarge_pixel,
                                     self.cfg.og_resolution.tabletop[0]+self.enlarge_pixel),device=self.device)
        self.table_expand_og_pre = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+self.enlarge_pixel,
                                     self.cfg.og_resolution.tabletop[0]+self.enlarge_pixel),device=self.device)
    ...
    def get_og(self,camera):
        ...
        og_boundary = (float(self.cfg.og_resolution.tabletop[0])/(200.0*self.cfg.og_resolution.scale),
                       float(self.cfg.og_resolution.tabletop[1])/(200.0*self.cfg.og_resolution.scale))
        ...
        Nx = self.cfg.og_resolution.tabletop[0]+self.enlarge_pixel
        Ny = self.cfg.og_resolution.tabletop[1]+self.enlarge_pixel
        # objects_pcd = o3d.geometry.PointCloud()
        # objects_pcd.points = o3d.utility.Vector3dVector(whole_point_cloud)
        # o3d.visualization.draw_geometries([objects_pcd])
        ################ original table
        occupancy_ex = np.zeros( (Ny,Nx) )
        occupancy_ex[int(self.enlarge_pixel/2):self.cfg.og_resolution.tabletop[1]+int(self.enlarge_pixel/2),
                     int(self.enlarge_pixel/2):self.cfg.og_resolution.tabletop[0]+int(self.enlarge_pixel/2)] = 1
        ...
        u = (Nx-int(self.enlarge_pixel)-1)*u +int(self.enlarge_pixel/2)
        v = (Ny-int(self.enlarge_pixel)-1)*v +int(self.enlarge_pixel/2)
        u = np.round(u).astype(int)
        v = np.round(v).astype(int)
        u_ind = np.where(u<Nx)
        u = u[u_ind]
...
class PushObservationManager(ObservationManager):
    ...
    def table_scene(self,env:PushEnv):
        # print("get observs")
        # obs_ta = torch.zeros((env.num_envs,env.cfg.og_resolution.tabletop[1]+int(env.enlarge_pixel),
        #                             env.cfg.og_resolution.tabletop[0]+int(env.enlarge_pixel),1),device=env.device)
        ...
        for i in range(env.num_envs):
            ...
            # observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1]+int(env.enlarge_pixel),
            #                         env.cfg.og_resolution.tabletop[0]+int(env.enlarge_pixel)])
...
class PushRewardManager(RewardManager):
    ...
    def penalizing_pushing_outside(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
            env_tab_ex_tmp[i][int(env.enlarge_pixel/2):env.cfg.og_resolution.tabletop[1]+int(env.enlarge_pixel/2),
                           int(env.enlarge_pixel/2):env.cfg.og_resolution.tabletop[0]+int(env.enlarge_pixel/2)] = 0
            env_tab_ex_tmp_pre[i][int(env.enlarge_pixel/2):env.cfg.og_resolution.tabletop[1]+int(env.enlarge_pixel/2),
                           int(env.enlarge_pixel/2):env.cfg.og_resolution.tabletop[0]+int(env.enlarge_pixel/2)] = 0
            if torch.sum(env_tab_ex_tmp[i])-torch.sum(env_tab_ex_tmp_pre[i])<=12:
                if env.check_reaching[i]>0.5:
    ...
    def reward_near_obj(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
                        ...
                        if torch.sum(table_og_tmp[j,start_ind_y:end_ind_y])>0:
                            ...
                            if len(ind_tmp)>0:
                                ...
                                if len(ind_tmp)>0:
                                    ...
                                    reward_near[i] -= np.min(np.abs(ind_pre_y.cpu().numpy()))*0.3
                                    # print(np.min(np.abs(ind_pre_y.cpu().numpy())))
                                else:
                                    reward_near[i] -= 10*0.3
                            else:
                                reward_near[i] -= 10*0.3
                            # print(min_reward)
            ...
            if torch.sum(table_og_tmp[env.actions_origin[i][0],start_ind_y:end_ind_y]) == 0:
                ...
                reward_near[i] -= np.min(np.abs(norm_dist))*0.3

...
from .push_cfg_1 import PushEnvCfg, RandomizationCfg, YCBobjectsCfg, CameraCfg
...
class PushEnv(IsaacEnv):
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        self.actions[:,:2] = self.actions[:,:2]-22
        actions_tmp = self.actions.clone()
        self.actions[:,2] = self.actions[:,2]/8.0
        
        action_range = (float(self.cfg.og_resolution.tabletop[0])/400.0,float(self.cfg.og_resolution.tabletop[1])/400.0)
    ...
    def _initialize_views(self) -> None:
        ...
        self.table_tsdf_pre = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1],
                                     self.cfg.og_resolution.tabletop[0]),device=self.device)
        self.table_expand_og = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+24,
                                     self.cfg.og_resolution.tabletop[0]+24),device=self.device)
        self.table_expand_og_pre = torch.zeros((self.num_envs,self.cfg.og_resolution.tabletop[1]+24,
                                     self.cfg.og_resolution.tabletop[0]+24),device=self.device)
    ...
    def get_og(self,camera):
        ...
        og_boundary = (float(self.cfg.og_resolution.tabletop[0])/400.0,float(self.cfg.og_resolution.tabletop[1])/400.0)
        ...
        Nx = self.cfg.og_resolution.tabletop[0]+int(0.06/(0.5/self.cfg.og_resolution.tabletop[0]))*2
        Ny = self.cfg.og_resolution.tabletop[1]+int(0.06/(0.5/self.cfg.og_resolution.tabletop[0]))*2
        # objects_pcd = o3d.geometry.PointCloud()
        # objects_pcd.points = o3d.utility.Vector3dVector(whole_point_cloud)
        # o3d.visualization.draw_geometries([objects_pcd])
        ################ original table
        shift_ind = int(0.06/(0.5/self.cfg.og_resolution.tabletop[0]))
        # print(shift_ind)
        # print(Nx,Ny)
        ...
        for i in range(self.num_envs):
            ...
            if torch.sum(table_og_tmp[self.actions_origin[i][0],start_ind_y:end_ind_y])>=1:
                ...
                if torch.sum(table_og_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y])==0:
                    self.check_reaching[i] = 1   
            '''
            if self.actions_origin[i][0] >=22 and self.actions_origin[i][0]<122 and self.actions_origin[i][1]>=22 and self.actions_origin[i][1]<122:
                self.actions_origin[i][0] -=22
                self.actions_origin[i][1] -=22
                # print(i,self.actions_origin[i])
                if (self.actions_origin[i,2]%2) == 0:
                    if (self.actions_origin[i,2]%4) == 0:
                        start_ind_x = max(self.actions_origin[i][0]-4,0)
                        end_ind_x = min(self.actions_origin[i][0]+5,self.cfg.og_resolution.tabletop[0])
                        start_ind_y = max(self.actions_origin[i][1]-3,0)
                        end_ind_y = min(self.actions_origin[i][1]+4,self.cfg.og_resolution.tabletop[1])
                    else:
                        start_ind_x = max(self.actions_origin[i][0]-3,0)
                        end_ind_x = min(self.actions_origin[i][0]+4,self.cfg.og_resolution.tabletop[0])
                    ...
                    if torch.sum(table_og_tmp[start_ind_x:end_ind_x,start_ind_y:end_ind_y])==0:
                        if self.actions_origin[i,2] == 0:
                            start_ind_y = max(self.actions_origin[i][1]-8,0)
                            end_ind_y = min(self.actions_origin[i][1],self.cfg.og_resolution.tabletop[1])
                            if torch.sum(table_og_tmp[self.actions_origin[i][0],start_ind_y:end_ind_y])>=1:
                                self.check_reaching[i] = 1 
                        elif self.actions_origin[i,2] == 2:
                            start_ind_x = max(self.actions_origin[i][0],0)
                            end_ind_x = min(self.actions_origin[i][0]+8,self.cfg.og_resolution.tabletop[0])
                            ...
                            end_ind_y = min(self.actions_origin[i][1]+8,self.cfg.og_resolution.tabletop[1])
                            if torch.sum(table_og_tmp[self.actions_origin[i][0],start_ind_y:end_ind_y])>=1:
                                self.check_reaching[i] = 1 
                        elif self.actions_origin[i,2] == 6:
                            start_ind_x = max(self.actions_origin[i][0]-8,0)
                            end_ind_x = min(self.actions_origin[i][0],self.cfg.og_resolution.tabletop[0])
                            if torch.sum(table_og_tmp[start_ind_x:end_ind_x,self.actions_origin[i][1]])>=1:
                                self.check_reaching[i] = 1  
                        if self.actions_origin[i][0] <= 5 or self.actions_origin[i][0] >= self.cfg.og_resolution.tabletop[1]-6:
                    ...
                    original_coords = original_coords-71
                    ...
                    new_x = int(round(new_coords[0, 0]))+71
                    new_y = int(round(new_coords[1, 0]))+71
                    # print('old : ', original_coords,'new: ',new_x, new_y)
                    start_ind_x = max(new_x-4,0)
                    end_ind_x = min(new_x+5,144)
                    start_ind_y = max(new_y-3,0)
                    end_ind_y = min(new_y+4,144)
                    table_og_tmp = self.table_og_pre[i].clone()
                    table_ex_tmp = torch.zeros((1,1,144,144)).cuda()
                    table_ex_tmp[0,0,22:122,22:122] = table_og_tmp.clone().detach()
                    ...
                    obs_space = rotate_table_ex_tmp[0,0].clone()
                    if torch.sum(obs_space[start_ind_x:end_ind_x,start_ind_y:end_ind_y])==0:
                        obs_space[start_ind_x:end_ind_x,start_ind_y:end_ind_y] = 2
                        start_ind_y = max(new_y-8,0)
                        end_ind_y = min(new_y,144)
                        if torch.sum(obs_space[self.actions_origin[i][0],start_ind_y:end_ind_y])>=1:
                            self.check_reaching[i] = 1
                        obs_space[start_ind_x:end_ind_x,start_ind_y:end_ind_y] = 3
                    else:
                        obs_space[start_ind_x:end_ind_x,start_ind_y:end_ind_y] = 2
                        start_ind_y = max(new_y-8,0)
                        end_ind_y = min(new_y,144)
                        obs_space[start_ind_x:end_ind_x,start_ind_y:end_ind_y] = 3
                    # plt.imshow(np.array(obs_space.cpu()))
                    # plt.show()
                    if self.actions_origin[i][0] <= 5 or self.actions_origin[i][0] >= self.cfg.og_resolution.tabletop[1]-6:
                            self.check_reaching[i] = 0
                    del obs_space, flow_grid_before,rotate_table_ex_tmp,table_og_tmp
    ...
    def _randomize_table_scene(self,env_ids: torch.Tensor):
        ...
        env_path = "train_table_42x42/"+file_name[choosen_env_id]
        # print(env_path)
        ...
        self.new_obj_mask[env_ids.tolist()] = self.cfg.obj_mask.mask[env[1]]
...
class PushObservationManager(ObservationManager):
    ...
    def table_scene(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
            ...
            # plt.show()
            observation = np.array(im,dtype=np.uint8)
            observation = observation[:,np.newaxis].reshape([env.num_envs,env.cfg.og_resolution.expand_obs[1],
                                    env.num_envs,env.cfg.og_resolution.expand_obs[1]+44])
            # observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1],
            #                         env.cfg.og_resolution.tabletop[0]])
            obs_ta[i,:,:,0] = torch.from_numpy(observation).to(env.device)
            # im_tsdf = env.table_tsdf_ex[i].cpu().numpy()*255/0.8
            if np.max(env.table_tsdf_ex[i].cpu().numpy())>0.00001:
                im_tsdf = env.table_tsdf_ex[i].cpu().numpy()*255/(np.max(env.table_tsdf_ex[i].cpu().numpy()))
                # print(np.max(env.table_tsdf_ex[i].cpu().numpy()))
            else:
                im_tsdf = env.table_tsdf_ex[i].cpu().numpy()*255
                
            observation = np.array(im_tsdf,dtype=np.uint8)
            observation = observation[:,np.newaxis].reshape([env.num_envs,env.cfg.og_resolution.expand_obs[1],
                                    env.num_envs,env.cfg.og_resolution.expand_obs[1]])
            # observation = observation[:,np.newaxis].reshape([env.cfg.og_resolution.tabletop[1],
            #                         env.cfg.og_resolution.tabletop[0]])
...
class PushRewardManager(RewardManager):
    ...
    def penaltizing_pushing_outside(self,env:PushEnv):
        ...
        for i in range(env.num_envs):
            ...
            # plt.show()
            env_tab_ex_tmp[i][12:env.cfg.og_resolution.tabletop[1]+12,
                           12:env.cfg.og_resolution.tabletop[0]+12] = 0
            env_tab_ex_tmp_pre[i][12:env.cfg.og_resolution.tabletop[1]+12,
                           12:env.cfg.og_resolution.tabletop[0]+12] = 0
            # fig, (ax1,ax2) = plt.subplots(1, 2, figsize=(7, 4))
            # ax1.imshow(env_tab_ex_tmp[i].cpu().numpy())
            # ax2.imshow(env_tab_ex_tmp_pre[i].cpu().numpy())
